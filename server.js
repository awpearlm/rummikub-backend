...
class RummikubGame {
  ...
  playSet(playerId, tileIds, setIndex = null) {
    ...
    if (!player || player.id !== this.getCurrentPlayer().id) {
      console.log(`Play rejected: not player's turn`);
      return false;
    }
    
    const tiles = tileIds.map(id => player.hand.find(t => t.id === id)).filter(Boolean);
    if (tiles.length !== tileIds.length) {
      console.log(`Play rejected: could not find all tiles in player's hand`);
      return false;
    }
    ...
    if (!this.isValidSet(tiles)) {
      console.log(`Server rejected set as invalid`);
      return false;
    }
